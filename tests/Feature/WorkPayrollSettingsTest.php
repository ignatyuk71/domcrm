...
class WorkPayrollSettingsTest extends TestCase
{
    ...
    public function test_piecework_employee_has_one_total_for_monthly_salary_and_manual_daywork(): void
    {
        ...
        $this->putJson("/settings/work-payroll/employees/$id", array_replace($payload, ['monthly_salary' => '9000']))->assertConflict();
        foreach (['-1', '1000001', '1.001', null] as $invalid) {
            $this->putJson("/settings/work-payroll/employees/$id", array_replace($payload, ['monthly_salary' => $invalid]))->assertUnprocessable();
        }
    }
    ...
    public function test_salary_carries_forward_but_not_backward_and_explicit_zero_stops_it(): void
    {
        ...
        $this->putJson("/settings/work-payroll/employees/$id", array_replace($payload, ['month' => '2026-10']))->assertOk();
        $this->putJson("/settings/work-payroll/employees/$id", array_replace($payload, ['monthly_salary' => '9000', 'version' => 1]))->assertOk();
