<template>
  <div class="right-sidebar">
    ...
    <div v-if="customerId" class="profile-container custom-scrollbar">
      ...
      <div class="history-container">
        ...
        <div v-else class="orders-list">
          <div v-for="order in historyOrders" :key="order.id" class="order-card" :class="{ 'is-open': order.isOpen }">
            <div class="order-header" @click="toggleOrder(order.id)">
              ...
              <div class="order-meta-info">
                <span class="order-price">{{ formatMoney(order.items_sum_total) }} ₴</span>
...
<script setup>
// ... JS ЛОГІКА ЗАЛИШАЄТЬСЯ БЕЗ ЗМІН ...
...
const toggleOrder = (orderId) => { const target = historyOrders.value.find((order) => order.id === orderId); if (target) target.isOpen = !target.isOpen; };
...
const createOrderFromDraft = async () => {
  ...
  try {
    ...
    orderSubmitState.orderId = order.id;
    orderSubmitState.orderNumber = order.order_number;
...
const handleOrderClose = () => {
  if (orderDraft.items.length > 0 && !confirm('Видалити чернетку?')) return;
  resetOrderDraft(); showOrderPanel.value = false;
};
</script>
...
.edit-icon { font-size: 12px; color: #cbd5e1; transition: 0.2s; }
.name-display:hover .edit-icon { color: #6366f1; }

.name-edit-group { display: flex; gap: 8px; }
.modern-input {
  border: 1px solid #e2e8f0; background: #f8fafc; border-radius: 8px;
  padding: 6px 10px; font-size: 14px; width: 100%; outline: none;
  transition: 0.2s;
}
.modern-input:focus { border-color: #6366f1; background: white; box-shadow: 0 0 0 2px rgba(99, 102, 241, 0.1); }
.btn-confirm-mini {
  background: #10b981; color: white; border: none;
  width: 32px; border-radius: 8px; flex-shrink: 0; cursor: pointer;
}
...
.section-title {
  ...
  color: #94a3b8; letter-spacing: 0.05em; margin-bottom: 16px; display: block;
}
...
.field-input {
  ...
  padding: 8px 12px; font-size: 14px; font-weight: 500; color: #0f172a;
  outline: none; transition: 0.2s;
}
.field-input:focus { border-color: #6366f1; }
.input-wrapper.error .field-input { border-color: #ef4444; background: #fef2f2; }
.btn-clear-field {
  position: absolute; right: 10px; top: 50%; transform: translateY(-50%);
  border: none; background: none; color: #cbd5e1; cursor: pointer; transition: 0.2s;
}
.btn-clear-field:hover { color: #ef4444; }

.btn-add-field {
  border: 1px dashed #cbd5e1; background: #f8fafc; width: 100%;
  padding: 8px; border-radius: 8px; color: #6366f1; font-size: 13px; font-weight: 500;
  cursor: pointer; transition: 0.2s;
}
.btn-add-field:hover { border-color: #6366f1; background: #eef2ff; }
...
.btn-primary-gradient {
  ...
  box-shadow: 0 4px 6px -1px rgba(99, 102, 241, 0.3); transition: 0.2s;
}
.btn-primary-gradient:hover { transform: translateY(-1px); box-shadow: 0 6px 8px -1px rgba(99, 102, 241, 0.4); }
...
.order-status-badge {
  font-size: 11px; font-weight: 600; padding: 2px 6px; border-radius: 4px; width: fit-content;
}

.order-meta-info { display: flex; align-items: center; gap: 12px; }
.order-price { font-weight: 700; color: #0f172a; font-size: 14px; }
.toggle-icon { font-size: 12px; color: #94a3b8; transition: transform 0.3s; }
...
.is-open .order-details-wrapper { max-height: 500px; }
...
.btn-full-details {
  ...
  background: white; border: 1px solid #cbd5e1; color: #4f46e5;
  font-size: 12px; font-weight: 600; padding: 8px; border-radius: 8px;
  text-decoration: none; transition: 0.2s; margin-top: 8px;
}
...
.toast-notification {
  ...
  border: 1px solid rgba(255,255,255,0.2); font-size: 13px; font-weight: 600; color: #1e293b;
  min-width: 200px;
}
.toast-icon {
