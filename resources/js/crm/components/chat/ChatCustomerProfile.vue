            ...
            >
              <button class="order-header" type="button" @click="toggleOrder(order.id)">
                ...
                <div class="header-right">
                  <!-- ВИПРАВЛЕНО: Ціна не переноситься -->
                  <div class="price-tag">
                    {{ formatPrice(order.items_sum_total) }}<small>&nbsp;грн</small>
                  </div>
...
// --- ВИПРАВЛЕНА ЛОГІКА СКРОЛУ ---
const toggleOrder = (orderId) => {
  const target = historyOrders.value.find((order) => order.id === orderId);
  if (target) {
    target.isOpen = !target.isOpen;
    // Скролимо тільки якщо відкрили
    if (target.isOpen) {
      // Чекаємо поки анімація майже завершиться (300мс CSS transition)
      setTimeout(() => {
...
<style scoped>
/* Стилі залишаються майже без змін, але поправлено header-right */
...
.order-header { padding: 12px 16px; cursor: pointer; display: flex; justify-content: space-between; align-items: flex-start; background: white; border: none; width: 100%; text-align: left; }
...
.order-status-badge { font-size: 10px; font-weight: 700; padding: 2px 8px; border-radius: 6px; width: fit-content; text-transform: uppercase; }

/* ВИПРАВЛЕНО: Header Right */
.header-right { 
  display: flex; 
  align-items: center; 
  gap: 8px; 
...
.toggle-icon { font-size: 12px; color: #cbd5e1; transition: transform 0.3s; margin-top: 2px; }
.is-open .toggle-icon { transform: rotate(180deg); color: #6366f1; }

.order-details-wrapper { max-height: 0; overflow: hidden; transition: max-height 0.4s cubic-bezier(0.4, 0, 0.2, 1); background: #f8fafc; border-top: 1px solid #f1f5f9; }
