<template>
  <section class="cost-page" :aria-busy="loading || saving || cardboardSaving || foamSaving || furSaving || laminateSaving || tapeSaving">
    ...
    <section v-else-if="activeComponent === 'labor'" class="cost-card cost-placeholder" data-testid="labor-info">
      <span class="cost-placeholder-icon"><i class="bi bi-tools" aria-hidden="true"></i></span>
      <h2>Робота</h2>
      <p>Оплата праці — поденна.</p>
      <strong class="labor-output">35 <span>готових пар на день</span></strong>
      <p>Орієнтир мінімального виробітку для двох швачок разом, по всіх моделях.</p>
      <span class="cost-state pending">Розрахунок відкладено</span>
      <p class="cost-placeholder-note">Вартість роботи поки не включена до собівартості пари.</p>
    </section>
    ...
    <SheetMaterialCostPanel ref="cardboardPanel" v-if="cardboardOpened" v-show="activeComponent === 'cardboard'" material="cardboard" @saving="cardboardSaving = $event" />
    <SheetMaterialCostPanel ref="foamPanel" v-if="foamOpened" v-show="activeComponent === 'foam'" material="foam" @saving="foamSaving = $event" />
    <FurCostPanel ref="furPanel" v-if="furOpened" v-show="activeComponent === 'fur'" @saving="furSaving = $event" />
    <LaminateCostPanel ref="laminatePanel" v-if="laminateOpened" v-show="activeComponent === 'laminate'" @saving="laminateSaving = $event" />
    <TapeCostPanel ref="tapePanel" v-if="tapeOpened" v-show="activeComponent === 'tape'" @saving="tapeSaving = $event" />
  </section>
</template>
...
<style scoped>
.labor-output{display:block;font-size:48px;font-weight:800;line-height:1.2;letter-spacing:-.03em;color:#574ce8;margin:24px 0 12px}.labor-output span{display:block;margin-top:8px;font-size:15px;font-weight:600;letter-spacing:0;color:#52647c}
