<template>
  <section class="sheet-cut panel-card" data-testid="cardboard-layout">
    <header><div><h3>Розкрій одного листа</h3><p>Заготовка {{ number(blankLength) }} × {{ number(blankWidth) }} см</p></div><i class="bi bi-grid-3x3" aria-hidden="true"></i></header>
    <template v-if="layout?.pairs">
      <figure><figcaption>{{ number(length) }} × {{ number(width) }} см</figcaption>
        <svg :viewBox="`-1 -1 ${length + 2} ${width + 2}`" role="img" :aria-label="`Картон: ${layout.primary_pieces} заготовок в основних рядах, ще ${layout.rotated_pieces} поперек. Усього ${layout.total_pieces} заготовок, ${layout.pairs} повних пар.${truncated ? ' Показано фрагмент.' : ''}`">
          <rect x="0" y="0" :width="length" :height="width" fill="#edf0f5" stroke="#cbd3df" stroke-width="1" vector-effect="non-scaling-stroke" />
          <polygon v-for="(polygon, index) in polygons" :key="index" :points="polygon.points.map(point => point.join(',')).join(' ')" :data-rotated="polygon.rotated" :fill="polygon.rotated ? '#f5cd8c' : '#bce5dc'" :stroke="polygon.rotated ? '#b67a23' : '#329a86'" stroke-width="0.7" vector-effect="non-scaling-stroke" />
        </svg>
        <div class="legend"><span><i class="main-color"></i>Основні ряди</span><span><i class="rotated-color"></i>Поперек у залишках</span><span><i class="offcut-color"></i>Обрізки</span></div>
      ...
      <p class="hint">Деталі в залишках повернуті на 90°. Це розкладка рядами без додаткових проміжків між прямокутниками.</p>
    </template>
