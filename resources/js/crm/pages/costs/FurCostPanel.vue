    ...
    <div v-if="form" class="panel-workspace">
      <form class="panel-card panel-form" @submit.prevent="save">
        <fieldset :disabled="saving">
          ...
          <section class="form-section">
            <h3><span>2</span> Закупівля, доставка та курси</h3>
            <div class="fields">
            ...
            <p class="hint">Сума лише за хутро, без підошви. Комісія — від хутра та доставки Китаєм. Митне оформлення вже в міжнародній доставці, вдруге не додаємо.</p>
            <p v-if="preview" class="commission">Комісія: ({{ money(decimalInput(form.goods_cny)) }} + {{ money(decimalInput(form.china_shipping_cny)) }}) × {{ number(decimalInput(form.commission_percent)) }}% = <b>{{ money(preview.commission_cny) }} ¥</b></p>
          </section>
          <section class="form-section">
          ...
          <details class="extra-fields"><summary>Назва, дата та додаткові витрати</summary><div class="fields">
            ...
            <label>Доставка по Україні<div class="input-group"><input v-model="form.ukraine_shipping_uah" name="ukraine_shipping_uah" inputmode="decimal" pattern="[0-9]+([.,][0-9]{1,2})?" placeholder="Не врахована" class="form-control" /><span class="input-group-text">грн</span></div></label>
            ...
            <p class="hint">Доставка лише цієї партії хутра. Невідому суму залиште порожньою.</p>
      ...
      <aside class="panel-result">
        ...
        <p v-else class="calculation-hint">Заповніть довжину, одиницю, ширину полотна, суми, курси та розміри трапеції. Деталь має поміщатися в полотно.</p>
...
import { calculateFurCost, furFields, emptyFurForm } from '@/crm/utils/furCosts';
...
function applyBatch(batch) {
  ...
  form.value = { name: batch.name, purchased_on: batch.purchased_on || '', note: batch.note || '', ...batch.inputs, ukraine_shipping_uah: batch.inputs.ukraine_shipping_uah ?? '' };
...
async function save() {
  ...
  if (!preview.value || !form.value.name.trim()) { formError.value = 'Перевірте назву, довжину, одиницю, суми, курси та розміри деталі.'; return; }
  saving.value = true; formError.value = ''; notice.value = '';
  const payload = { name: form.value.name.trim(), purchased_on: form.value.purchased_on || null, note: form.value.note.trim() || null, length_unit: form.value.length_unit,
    ...Object.fromEntries(furFields.map(field => [field, field === 'ukraine_shipping_uah' && decimalInput(form.value[field]) === '' ? null : decimalInput(form.value[field])])),
