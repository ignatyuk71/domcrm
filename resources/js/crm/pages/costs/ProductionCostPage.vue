<template>
  <section class="cost-catalog" :aria-busy="loading || opening !== null">
    <header class="catalog-heading" :class="{ 'with-summary': activeModel }">
      <div>
...
import { costSummaryKey } from '@/crm/composables/useCostSummaryPart';
...
const summaryParts = ref({});
watch(() => activeModel.value?.id, () => { summaryParts.value = {}; }, { flush: 'sync' });
provide(costSummaryKey, (modelId, component, snapshot) => {
  if (modelId === activeModel.value?.id) summaryParts.value = { ...summaryParts.value, [component]: snapshot };
});
...
<style scoped>
.catalog-heading.with-summary{align-items:center}.catalog-heading.with-summary>div{flex:1;min-width:0}.catalog-heading.with-summary h1{overflow-wrap:anywhere}@media(max-width:900px){.catalog-heading.with-summary{flex-direction:column;align-items:stretch;gap:18px}}
