<template>
  <section class="cut-card" :class="shape" :data-testid="`laminate-${part}-layout`">
    ...
    <div v-if="layout?.pairs" class="cut-content">
      <figure><figcaption>100 × {{ number(width) }} см</figcaption><svg :viewBox="`-1 -1 102 ${Number(width) + 2}`" role="img" :aria-label="`${title}: ${layout.pieces_per_row} у ряду, ${layout.rows_per_cut} рядів, ще ${layout.rotated_pieces} поперек у залишках. Усього ${layout.total_pieces} деталей. Сіре — обрізки.${truncated ? ' Показано фрагмент.' : ''}`">
        <rect x="0" y="0" width="100" :height="width" fill="#edf0f5" stroke="#cbd3df" stroke-width="1" vector-effect="non-scaling-stroke" />
        <polygon v-for="(polygon, index) in polygons" :key="index" :points="polygon.points.map(point => point.join(',')).join(' ')" :data-rotated="polygon.rotated" :fill="polygon.rotated ? '#f5cd8c' : shape === 'rectangle' ? '#bce5dc' : polygon.flipped ? '#bcb3f6' : '#ded9fb'" :stroke="polygon.rotated ? '#b67a23' : shape === 'rectangle' ? '#329a86' : '#7361ed'" stroke-width="0.7" vector-effect="non-scaling-stroke" />
      </svg><small>{{ truncated ? 'Фрагмент розкладки. ' : '' }}Сіре — обрізки<span v-if="layout.rotated_pieces" class="extra-legend">Жовте — поперечні деталі</span></small></figure>
      <dl>
        <div><dt>У ряду</dt><dd>{{ number(layout.pieces_per_row) }}</dd></div>
        <div><dt>Рядів</dt><dd>{{ number(layout.rows_per_cut) }}</dd></div>
        <div><dt>В основних рядах</dt><dd>{{ number(layout.primary_pieces) }}</dd></div>
        <div class="extra-count"><dt>Поперек у залишках</dt><dd :data-testid="`laminate-${part}-rotated`">+{{ number(layout.rotated_pieces) }}</dd></div>
    ...
    <p class="cut-hint">У вільних смугах додаємо деталі, повернуті на 90°. Такий розкрій підходить, якщо напрям плюшу дозволяє поворот.</p>
  </section>
</template>
...
const polygons = computed(() => laminatePolygons(props.layout, props.top, props.bottom, props.height));
</script>

<style scoped>
.extra-legend{display:block;color:#986519;margin-top:4px}.extra-count dt,.extra-count dd{color:#986519}
