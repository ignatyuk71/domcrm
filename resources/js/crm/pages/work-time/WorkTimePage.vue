<template>
  <section class="work-time" :aria-busy="loading">
      ...
      <div v-if="ready && employees.length" class="wt-scroll">
        <table class="wt-calendar" :style="{ '--wt-day-count': days.length }" aria-label="Години роботи працівників за днями">
          ...
          <tbody><tr v-for="employee in employees" :key="employee.id">
            <th class="wt-person" scope="row"><div class="wt-person-actions"><button class="wt-person-button" type="button" :data-testid="`employee-${employee.id}`" :disabled="loading" @click="openPerson(employee)"><span class="wt-avatar">{{ employee.name.slice(0, 1) }}</span><span><b>{{ employee.name }}</b><small>{{ employee.archived_on ? 'В архіві' : employee.position || 'Працівник' }}</small></span></button><button v-if="owner" class="wt-delete" type="button" :data-testid="`delete-employee-${employee.id}`" :aria-label="`Видалити працівника ${employee.name}`" title="Видалити працівника" :disabled="loading || piece.loading || deletePreparing" @click="openDelete(employee)"><i class="bi bi-trash3" aria-hidden="true"></i></button></div></th>
        ...
        <table class="wt-calendar" :style="{ '--wt-day-count': days.length }" aria-label="Суми за виконану роботу за днями">
          ...
          <tbody><tr v-for="employee in piece.employees" :key="employee.id">
            ...
            <td v-for="day in days" :key="day.date" :class="{ weekend: day.weekend, 'wt-cell-error': piece.states[key(employee.id, day.date)]?.error, 'wt-cell-saving': piece.states[key(employee.id, day.date)]?.pending }">
              <input v-model="piece.drafts[key(employee.id, day.date)].amount" type="text" inputmode="decimal" maxlength="13" :data-money-cell="key(employee.id, day.date)" :title="piece.drafts[key(employee.id, day.date)].amount ? `${piece.drafts[key(employee.id, day.date)].amount} грн` : ''" :aria-label="`${employee.name}, ${day.day} ${monthGen[month - 1]}, сума у гривнях`" :aria-invalid="!!piece.states[key(employee.id, day.date)]?.error" :disabled="loading || piece.loading || !!(employee.archived_on && day.date > employee.archived_on)" @input="piece.schedule(key(employee.id, day.date))" @blur="piece.flush(key(employee.id, day.date))" @keydown.enter.prevent="nextPieceEmployee(employee.id, day.date)" />
    ...
    <dialog ref="deleteDialog" class="wt-dialog wt-delete-dialog" aria-labelledby="wt-delete-title" aria-describedby="wt-delete-description" @cancel.prevent="closeDelete">
      <template v-if="deleteTarget">
        <header class="wt-dialog-header"><div><span class="wt-eyebrow">ПОВНЕ ВИДАЛЕННЯ</span><h2 id="wt-delete-title">Видалити працівника?</h2></div><button class="wt-close" type="button" aria-label="Закрити підтвердження" :disabled="deleteSaving" @click="closeDelete"><i class="bi bi-x-lg" aria-hidden="true"></i></button></header>
        <p class="wt-delete-name">{{ deleteTarget.name }}</p>
        <p id="wt-delete-description">Буде видалено працівника та всі його години, суми за виконану роботу, нарахування й історію змін <strong>за всі місяці</strong>.</p>
        <p class="alert alert-warning">Це не архівація. Відновити дані через CRM буде неможливо. Інші працівники залишаться без змін.</p>
        <div v-if="deleteError" class="alert alert-danger" role="alert">{{ deleteError }}</div>
        <footer class="wt-dialog-footer"><button ref="deleteCancel" class="btn btn-outline-secondary" type="button" :disabled="deleteSaving" @click="closeDelete">Скасувати</button><button class="btn btn-danger" type="button" data-testid="confirm-delete-employee" :disabled="deleteSaving" @click="confirmDelete">{{ deleteSaving ? 'Видаляємо…' : 'Видалити назавжди' }}</button></footer>
      </template>
    </dialog>
    <p v-if="deleteNotice" class="wt-bottom-note" role="status">{{ deleteNotice }}</p>
  </section>
</template>
...
import { createWorkEmployee, updateWorkEmployee, deleteWorkEmployee, fetchWorkPayroll, saveWorkPayroll } from '../../services/workTimeApi';
...
const { period, days, employees, loading, ready, loadError, canManage, entries, drafts, states, pending, unsaved, failures,
  key, employeeHours, employeeDays, allHours, dayHours, lastDay, load: loadHours, flush, schedule, flushAll, forgetEmployee } = useWorkTime();
...
const deleteDialog = ref(), deleteCancel = ref(), deleteTarget = ref(null), deleteSaving = ref(false), deletePreparing = ref(false), deleteError = ref(''), deleteNotice = ref('');
let deleteFocus = null;
async function openDelete(employee) {
  if (!owner.value || deletePreparing.value || deleteSaving.value) return;
  deleteFocus = document.activeElement; deletePreparing.value = true;
  try {
    deleteTarget.value = { ...employee }; deleteError.value = ''; deleteNotice.value = '';
    await showDialog(deleteDialog); deleteCancel.value?.focus();
  } finally { deletePreparing.value = false; }
}
function closeDelete() {
  if (deleteSaving.value) return;
  deleteDialog.value.close(); deleteTarget.value = null; deleteFocus?.focus();
}
async function confirmDelete() {
  if (!owner.value || !deleteTarget.value || deleteSaving.value) return;
  deleteSaving.value = true; deleteError.value = '';
  try {
    // Модальне вікно вже блокує новий ввід: спершу завершуємо всі автозбереження.
    if (!await flushBoth()) { deleteError.value = 'Спочатку виправте незбережені клітинки в таблицях. Працівника не видалено.'; return; }
    const { id } = deleteTarget.value;
    await deleteWorkEmployee(id, deleteTarget.value.version);
    // Прибираємо локальні чернетки лише після підтвердження сервера.
    forgetEmployee(id); piece.forgetEmployee(id);
    deleteNotice.value = 'Працівника та всі його записи видалено. Відновлення через CRM недоступне.';
    deleteDialog.value.close(); deleteTarget.value = null;
    await nextTick(); document.querySelector('.wt-heading button')?.focus();
  } catch (error) { deleteError.value = workError(error); }
...
const protectPay = event => { if (payDirty.value || paySaving.value || deleteSaving.value) { event.preventDefault(); event.returnValue = ''; } };
...
<style scoped>
.wt-person-actions{display:flex;align-items:center;gap:3px}.wt-person-actions .wt-person-button{min-width:0;flex:1}.wt-person-actions .wt-avatar{display:none}.wt-delete{border:0;background:transparent;color:#8a94a7;flex-shrink:0;width:28px;height:34px;border-radius:7px;font-size:13px}.wt-delete:hover,.wt-delete:focus-visible{color:#bc314c;background:#fff0f2}.wt-delete-dialog p{font-size:14px;line-height:1.6}.wt-delete-name{font-weight:700;overflow-wrap:anywhere}.wt-delete-dialog .wt-eyebrow{color:#bc314c}
...
.wt-calendar .wt-person-button{gap:7px}
.wt-calendar .wt-person-button b{font-size:12px}
...
@container (max-width:1150px){
  ...
  .wt-calendar .wt-person-button small{font-size:10px}
