        ...
        <tbody v-for="group in groups" :key="group.type">
          ...
          <tr v-for="state in group.rows" :key="state.server.employee_id" :data-testid="`payroll-row-${state.server.employee_id}`" :class="{ 'ledger-error': state.error }">
            ...
            <td v-if="group.type === 'hourly'" class="ledger-editable ledger-rate">
              ...
              <select :value="state.values.rate_mode" :aria-label="`Одиниця ставки — ${state.server.employee.name}`" :disabled="disabled" @change="edit(state, 'rate_mode', $event.target.value); save(state)"><option value="hourly">за годину</option><option value="daily">за 8 годин</option></select>
