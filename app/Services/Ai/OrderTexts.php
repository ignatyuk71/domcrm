<?php

namespace App\Services\Ai;

/**
 * Тексти й реквізити флоу замовлення. Поки в коді (легко редагувати тут);
 * згодом можна винести в налаштування. Номер картки шлеться ЛИШЕ звідси —
 * щоб модель випадково не переплутала цифру.
 */
class OrderTexts
{
    public static function all(): array
    {
        return [
            'delivery_template' =>
                "Для оформлення замовлення вкажіть, будь ласка, наступні дані 📝\n"
                . "🚚 Доставка до вашого відділення пошти!\n\n"
                . "📌 ПІБ отримувача\n"
                . "📌 Номер мобільного\n"
                . "📌 Адреса доставки (місто/село, номер відділення пошти)\n"
                . "📌 Оплата: при отриманні або передоплата (картка/ФОП)",
            'payment_question' =>
                "Є два варіанти оплати 💰 — при отриманні або передоплата (на картку чи за реквізитами ФОП). Як вам зручніше?",
            // Передоплата шлеться ОБОМА способами разом (картка + ФОП), клієнт обирає сам.
            // Номер картки й IBAN — завжди ОКРЕМИМИ повідомленнями (зручно копіювати, модель їх НЕ друкує).
            'card_intro' =>
                "💳 Оплата на картку ПриватБанк (ФОП Ігнатюк Л.В):",
            'card_number' => '5169335107343648',
            // Реквізити рахунку ФОП. Сума підставляється з замовлення (%d грн).
            'iban_intro' =>
                "🧾 Або за реквізитами ФОП:\n"
                . "Отримувач: ІГНАТЮК ЛЮДМИЛА ВОЛОДИМИРІВНА\n"
                . "РНОКПП: 2417411841\n"
                . "Призначення: за товар\n"
                . "Сума до оплати: %d грн\n\n"
                . "Оберіть зручний спосіб 🙂 Після оплати перешліть, будь ласка, квитанцію або скрін 🙏",
            'iban_number' => 'UA133052990000026009010716418',
            'final_message' =>
                "Дякуємо за замовлення 💛 Відправка протягом 1-2 днів, чекайте на ТТН ♥️",
            'delivery_time' => '1-2 дні від дати замовлення',
            // Без прив'язки до часу доби: догонялка часто йде ввечері,
            // і «Доброго дня ще раз» о 22:43 виглядало дивно (аудит 06.07).
            'follow_up' =>
                "Вітаю ще раз 🙂 Підкажіть, чи вдалося визначитися з вибором? Радо допоможу з розміром чи кольором 💛",
            'voice_reject' =>
                "Дякую за повідомлення 🙂\n"
                . "На жаль, голосові я не можу прослухати — напишіть, будь ласка, текстом, і я залюбки допоможу 💛",
            'handover' =>
                "Хвилинку, уточню це питання й повернусь до вас 🙂",
        ];
    }
}
