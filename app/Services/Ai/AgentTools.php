...
class AgentTools
{
    ...
    public function toolAskDeliveryDetails(InboxConversation $conversation): array
    {
        ...
        return $ok
            ? ['готово' => 'Форму даних доставки надіслано клієнту. Дочекайся ПІБ, телефон і адресу. Спосіб оплати клієнт може вказати одразу у формі — тоді НЕ перепитуй його; не вказав — спитай окремо. Не друкуй цю форму ще раз.']
    ...
    public function toolSendPaymentDetails(InboxConversation $conversation, array $input = []): array
    {
        ...
        return ['готово' => 'Надіслано ОБИДВА способи: картка (номер окремо) і реквізити ФОП із сумою (IBAN окремо) — клієнт обере сам. НЕ друкуй номер картки чи IBAN сам, НЕ переказуй зміст реквізитів («оберіть спосіб», «скиньте квитанцію» — там уже написано). Дані доставки: якщо клієнт їх УЖЕ надав — нічого не проси; якщо ще НІ — нагадай ОДНИМ коротким рядком («І напишіть, будь ласка, дані для доставки — можна прямо сюди 🙂»), форму зі списком ПІБ/телефон/адреса ПОВТОРНО НЕ друкуй (вона вже в чаті). Чекай скрін/квитанцію оплати, потім зафіксуй замовлення через complete_order.'];
    }
