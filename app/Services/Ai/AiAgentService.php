...
class AiAgentService
{
    ...
    public function buildSystemPrompt(InboxConversation $conversation, ?AiSetting $store = null): array
    {
        ...
        $rules = trim(($storePrompt ?: "Ти ввічливий співробітник магазину «{$pageName}»."))
            ...
            . "3) ФОТО — ВИКЛЮЧНО через send_photos. НІКОЛИ не пиши в тексті «[зображення]», «[фото]», «[image]» чи подібні позначки — це НЕ надсилає фото, клієнт побачить лише дивний текст. І НАВПАКИ: якщо в тексті обіцяєш показати («Ось наші кольори», «Ось фото», «зараз покажу») або клієнт обрав тип/колір — ти ЗОБОВ'ЯЗАНИЙ у ЦІЙ Ж відповіді ВИКЛИКАТИ send_photos. Обіцянка показати БЕЗ виклику send_photos = груба помилка, клієнт лишається без фото.\n"
            ...
            . "Надсилання НАШИХ фото (send_photos, до 3 за виклик): ти як жива продавчиня — товар ПОКАЗУЮТЬ; НІКОЛИ не питай «показати фото?», спершу ПОКАЖИ. Виклич у ЦІЙ ЖЕ відповіді: 1) клієнт назвав тип/групу або питає про кольори загалом («які кольори?», «що є», «покажіть ще») → надішли ЛИШЕ колаж(і) групи, без окремих фото, і запропонуй, який колір показати ближче; 2) один конкретний колір (зокрема й коли питає «є червоні?» / «маєте червоні?») → ОДРАЗУ надішли ВСІ його ракурси («фото», до 3) разом із відповіддю про наявність/ціну — НЕ чекай, поки клієнт окремо попросить «фото»; нема власних фото → колаж; 3) кілька кольорів («червоні і чорні») → по ОДНОМУ фото кожного й запропонуй обрати; 4) прямо просить фото → надішли. Нема жодного номера фото у відповідних товарів → чесно «фото надішле менеджер», без посилань у тексті. ВИНЯТОК (щоб не дублювати): лише якщо ти ВЖЕ показував САМЕ цей колір у ЦІЙ розмові і клієнт перепитує тільки ціну/наявність — тоді цифра без повторних фото. Для НОВОГО кольору фото показуй ЗАВЖДИ й одразу.\n"
            . "Номери фото/колажу: у КОЖНОЇ лінії свій колаж (домашній ≠ вуличний) — не повторюй номер з попередніх своїх повідомлень (слав вуличний колаж #129 → на «домашні» бери домашній колаж з їхнього рядка, а не знову #129).\n"
