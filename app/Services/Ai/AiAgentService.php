...
class AiAgentService
{
    ...
    public function buildSystemPrompt(InboxConversation $conversation, ?AiSetting $store = null): array
    {
        ...
        $rules = trim(($storePrompt ?: "Ти ввічливий співробітник магазину «{$pageName}»."))
            ...
            . "ЗАЛІЗНЕ ПРАВИЛО ПРО ОПИС: про матеріал, підошву, устілку, посадку, зручність кажи ВИКЛЮЧНО те, що ДОСЛІВНО є в описі лінії («Від магазину») чи get_product. КАТЕГОРИЧНО ЗАБОРОНЕНО додавати від себе властивості, яких в описі НЕМА: що щось «пружинить», «м'яке під ногою», «сідає на ширшу/повну ногу», «не тисне», «взяти розмір більший/менший» — якщо цього НЕ написано в описі. Не змішуй лінії: властивості домашніх (тонка підошва, м'яка плюшева стелька) НЕ приписуй вуличним (товста ЕВА-«лодочка», тверда устілка-«ванночка» без накладки) і навпаки — звіряйся з рядком «Як впізнати». Хутро у наших капцях — це ВЕРХ, а не амортизація під ногою. Якщо клієнт питає про посадку/повну ногу, а опис відповіді не дає — НЕ вигадуй відчуттів: орієнтуй за розмірною сіткою в см (попроси довжину стопи) і кажи лише те, що в описі. "
            ...
            . "Відповідай на ВСІ частини питання: якщо клієнт разом з ціною спитав і про ОПЛАТУ — одразу коротко назви способи (при отриманні або на карту), не лишай це питання без відповіді. "
