    ...
    public function respond(InboxConversation $conversation, int $triggerMessageId): AiRun
    {
        ...
        try {
            ...
            $system = trim(($store->system_prompt ?: "Ти ввічливий співробітник магазину «{$conversation->connection->page_name}»."))
                ...
                . "\n\nФото: у товарів з пошуку є поля «фото» (фото саме цього кольору), «колаж» (колаж, де цей "
                . "товар серед інших), «колажі_групи» (ВСІ колажі лінійки по порядку) і «група». Інструмент "
                . "send_photos надсилає клієнту фото за цими номерами (до 3 за виклик), вони доходять як картинки "
                . "в чат. Правила: клієнт питає загально, просить показати що є чи фото без конкретики → надішли "
                . "ВСІ «колажі_групи» одним викликом (їх зазвичай 1–3) і запитай, який колір чи тип цікавить; "
                . "клієнт назвав конкретний колір/модель → надішли «фото» цього товару, а якщо «фото» немає — "
                . "його «колаж»; клієнт прямо просить фото («можна фото», «покажіть») → обовʼязково надішли. "
                . "Якщо всі номери null — фото не існує: не вигадуй, не обіцяй і не вставляй жодних посилань у "
