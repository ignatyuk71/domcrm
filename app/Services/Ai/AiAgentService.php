    ...
    public function respond(InboxConversation $conversation, int $triggerMessageId): AiRun
    {
        ...
        try {
            ...
            if (collect($toolsCalled)->contains(fn ($t) => ($t['tool'] ?? '') === 'complete_order')) {
                // Текст моделі НЕ ріжемо (вимога власника 20.07 після кейсу conv 599:
                // викинута відповідь на «які ще є тапки» лишила клієнта без відповіді).
                // Все, що написав агент, іде клієнту; канонічний фінал — окремим
                // абзацом після. Від дубля фіналу захищає промпт (крок 5), не код.
                $final = self::orderTexts()['final_message'];
                $text = trim($text) === '' ? $final : trim($text) . "\n\n" . $final;
