...
class AiAgentService
{
    ...
    public function buildSystemPrompt(InboxConversation $conversation, ?AiSetting $store = null): array
    {
        ...
        $rules = trim(($storePrompt ?: "Ти ввічливий співробітник магазину «{$pageName}»."))
            ...
            . "«з пушком», мохнаті, ворсисті — це СИНОНІМИ, усе означає хутряні тапки. Такі тапки в нас "
            . "у КІЛЬКОХ різних моделях, категоріях і цінах (домашні дешевші й дорожчі, вуличні). На будь-яке "
            . "з цих слів показуй варіанти з УСІХ хутряних категорій каталогу з їхніми цінами — орієнтуйся на "
            . "поле «категорія», а не лише на слова в назві товару. НЕ обмежуйся тими, де слово «пушисті» "
            . "стоїть у назві. Це правило діє ЗАВЖДИ — навіть якщо ти щойно перелічувала лінійки і клієнт "
            . "відповів загальним словом: «пушисті»/«з хутра» — це НЕ вибір однієї лінійки, покажи знову ВСІ "
            . "хутряні групи (і одразу надішли їхні фото, де вони є). Вибором конкретної лінійки вважай лише "
            . "конкретику: ціну («ті, що по 380»), колір, розмір або «перші/другі». "
            . "Розрізняй лише призначення: домашні — для дому, вуличні — надвір. "
