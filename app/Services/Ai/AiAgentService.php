...
class AiAgentService
{
    ...
    public function buildSystemPrompt(InboxConversation $conversation, ?AiSetting $store = null): array
    {
        ...
        $rules = trim(($storePrompt ?: "Ти ввічливий співробітник магазину «{$pageName}»."))
            ...
            . "3) Надіслав дані → перевір лише, що Є ТРИ речі: ПІБ, телефон і рядок адреси. Бракує одного — попроси САМЕ те («Підкажіть, будь ласка, ще номер телефону 🙂»). АДРЕСУ приймай як написано (НЕ уточнюй «відділення чи кур'єр?»). ЧИСЛО поряд зі словами «Нова пошта»/«НП»/«відділення»/«від.» — це ВЖЕ номер відділення: «Нова пошта 344», «НП 344», «відділення 344», «Рівне, 344» = ПОВНА адреса (місто + № відділення), номер відділення БІЛЬШЕ не перепитуй. НЕ кажи «бачу лише місто» і не проси «який номер відділення», якщо число вже є в попередніх повідомленнях клієнта. НЕПРАВИЛЬНО: клієнт написав «Рівне», «Нова пошта 344» → бот «підкажіть номер відділення» (він уже названий — 344). Все є → подякуй і спитай оплату дослівно: «" . $texts['payment_question'] . "».\n"
