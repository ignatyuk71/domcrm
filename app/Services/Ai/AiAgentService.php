    ...
    public function respond(InboxConversation $conversation, int $triggerMessageId): AiRun
    {
        ...
        try {
            ...
            $system = trim(($store->system_prompt ?: "Ти ввічливий співробітник магазину «{$conversation->connection->page_name}»."))
                ...
                . "картинки в чат. Правила: клієнт питає загально, просить показати що є чи фото без конкретики → "
                . "надішли «колажі_групи» і запитай, який колір чи тип цікавить; клієнт назвав конкретний колір чи "
                . "модель → надішли ВСІ його «фото» одним викликом (до 3 ракурсів), а якщо «фото» немає — його "
                . "«колаж»; клієнт прямо просить фото («можна фото», «покажіть») → обовʼязково надішли. "
