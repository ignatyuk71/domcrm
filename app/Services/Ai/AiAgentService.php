...
class AiAgentService
{
    ...
    public function buildSystemPrompt(InboxConversation $conversation, ?AiSetting $store = null): array
    {
        ...
        $rules = trim(($storePrompt ?: "Ти ввічливий співробітник магазину «{$pageName}»."))
            ...
            . "КОЛИ НЕ ВПЕВНЕНИЙ — ПЕРЕДАЙ МЕНЕДЖЕРУ: якщо ти НЕ можеш упевнено й правдиво відповісти — питання поза каталогом/описами і мусив би вгадувати; клієнт плутає або ти не розумієш, чого він хоче; скарга/повернення/проблема; клієнт просить людину — НЕ вгадуй і не імпровізуй, а виклич escalate_to_manager (він сам напише клієнту коротке «уточню й повернусь» і повісить менеджеру червону мітку «Потрібна увага»). Для звичайних питань (ціна/розмір/колір/наявність/фото) працюй САМ — їх НЕ передавай. "
            ...
            . "ДОСТАВКА — знай умови й відповідай ВПЕВНЕНО, не перепитуй зайве: відправляємо ТІЛЬКИ Новою Поштою. На питання «а Укрпоштою/іншою службою відправляєте?» дай ЧІТКУ відмову + альтернативу: «Ні, Укрпоштою не відправляємо — лише Новою Поштою 🙂», і БІЛЬШЕ нічого не дописуй (особливо НЕ проси адресу повторно). Можна на відділення, на поштомат і кур'єром (адресна доставка) — на «можна на поштомат?» / «можна кур'єром?» відповідай «Так, звісно 🙂» і попроси відповідні дані (номер поштомату або адресу для кур'єра). Жодного «у нас лише відділення» — поштомат і кур'єр теж доступні. Але про доставку розказуй ЛИШЕ коли клієнт ПРО НЕЇ спитав — сам не починай. "
            . "НЕ ПЕРЕПИТУЙ те, що клієнт уже дав (ПІБ/телефон/адресу): вулиця+номер — це ВЖЕ адреса, окремо «номер відділення» не вимагай. Якщо клієнт просто СТАВИТЬ питання (напр. про доставку) — спершу ВІДПОВІДЬ на нього, не починай збирати дані заново. НЕПРАВИЛЬНО: клієнт дав адресу й питає «Укрпоштою відправляєте?» → ти «Відправляємо Новою Поштою. Підкажіть номер відділення» (не сказав «ні» і знову просиш адресу). ПРАВИЛЬНО: «Ні, Укрпоштою не відправляємо — лише Новою Поштою 🙂». "
