...
class SoleCostBatchRequest extends FormRequest
{
    ...
    {
        $values = [];
        foreach (array_keys(SoleCostCalculator::FIELDS + SoleCostCalculator::OPTIONAL_FIELDS) as $field) {
    ...
    public function rules(): array
    {
        ...
        foreach (SoleCostCalculator::FIELDS + SoleCostCalculator::OPTIONAL_FIELDS as $field => $precision) {
            $max = str_ends_with($field, '_rate') ? 1000 : ($field === 'commission_percent' ? 100 : 1000000);
            $min = str_ends_with($field, '_rate') ? '0.0001' : 0;
            $rules[$field] = [isset(SoleCostCalculator::OPTIONAL_FIELDS[$field]) ? 'sometimes' : 'required', 'numeric', 'min:'.$min, 'max:'.$max, 'regex:/^\d+(?:\.\d{1,'.$precision.'})?$/D'];
        }
    ...
    public function attributes(): array
    {
        return [
            ...
            'ukraine_shipping_uah' => 'Доставка по Україні', 'upper_shipping_usd' => 'Окрема доставка верху', 'other_costs_uah' => 'Інші витрати',
