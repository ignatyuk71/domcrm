...
## Оновлення оплати WayForPay

Після перевіреного серверного підтвердження платежу сайт повторно надсилає запит на той самий endpoint, з тим самим `external_order_id`, API-ключем та HMAC-підписом. Повернення покупця на сторінку успіху не є підтвердженням платежу.

```json
{
  "external_order_id": "4987",
  "payment": {
    "method": "card",
    "provider": "wayforpay",
    "status": "paid",
    "paid_amount": 5.00,
    "currency": "UAH",
    "transaction_id": "ідентифікатор-платежу-на-сайті",
    "paid_at": "2026-09-22T08:00:00Z"
  }
}
```

- Початкове створення замовлення, як і раніше, потребує повного payload із товарами. Оновлення лише оплати надсилайте після підтвердження приймання початкового замовлення.
- `payment.status`: `unpaid`, `prepayment`, `paid`. Сам спосіб `card` чи провайдер `wayforpay` не підтверджує оплату. Для `paid` і `prepayment` передавайте `paid_amount` — загальну сплачену суму за товар, а не приріст останнього платежу чи суму за мінусом комісії.
- Валюта платежу має збігатися з валютою замовлення. Доставка, яку отримувач оплачує перевізнику, до цієї суми не входить.
- `provider`, `transaction_id`, `paid_at` — необов'язкові реквізити; час передавайте з часовим поясом. `method: wayforpay` також підтримується і нормалізується у `card` + `provider: wayforpay`.
- Повторний запит оновлює лише оплату, не створює іншого замовлення й не перезаписує товари, адресу, нотатки або робочий статус. Незмінний оброблений запит повертає `200` із `duplicate: true`; нові дані — `202` зі статусом обробки.
- Запізнілі `unpaid`/часткові платежі не скасовують повну оплату й не зменшують сплачену суму. Цей контракт не виконує повернень; `refund` у CRM залишається захищеним від таких оновлень.
- `202 accepted` означає приймання, а не гарантію успішної обробки: `status: failed` потребує виправлення даних і повторного надсилання. При асинхронній черзі стан видно в журналі інтеграції CRM.
- Фіскалізація, чеки, її передоплата та автоматичні фіскальні завдання не змінюються і не запускаються цим оновленням.

### Увімкнення та відновлення старої оплати

Після розгортання виконайте `php artisan migrate --force`, зберіть frontend (`npm run build`) та перезапустіть працівники черги (`php artisan queue:restart`). Сайт має надсилати наведені поля як під час створення вже оплаченого замовлення, так і після пізнішого підтвердження платежу.

Якщо старий збережений payload уже містить `payment.status` (або верхньорівневий `payment_status`) та `payment.paid_amount`, перевірте конкретне замовлення:

```sh
php artisan integrations:sync-payment 6527
php artisan integrations:sync-payment 6527 --apply
```

Перша команда перевіряє дані без збереження, друга застосовує оплату. Без підтверджених даних команда відмовляється позначати замовлення оплаченим: тоді потрібне повторне надсилання із сайту. Скриншот або сам спосіб оплати для цього недостатні.
