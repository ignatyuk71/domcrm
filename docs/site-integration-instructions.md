...
## Синхронізація підтвердженої онлайн-оплати

Для WayForPay сайт повторно надсилає оплату на `/api/v1/orders/intake` з тим самим `external_order_id`. Передавайте `payment.provider: wayforpay`, `payment.status: paid`, фактичну `payment.paid_amount` та валюту після перевіреного серверного підтвердження. Самого `payment.method: card` недостатньо. Повний контракт і відновлення старих оплат описані в [інструкції формату інтеграції](integration-site-format.md#оновлення-оплати-wayforpay).
